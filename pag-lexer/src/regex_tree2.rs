...
pub enum RegexTree<'a> {
    Top,    // any character
    Bottom, // no character
    Set(Intervals),
    Epsilon,
    Concat(SmallVec<[&'a Self; 2]>),
    KleeneClosure(&'a Self),
    Union(SmallVec<[&'a Self; 2]>),
    Intersection(SmallVec<[&'a Self; 2]>),
    Complement(&'a Self),
}

use RegexTree::*;

impl Display for RegexTree<'_> {
...
const EPSILON: RegexTree = RegexTree::Epsilon;
const BOTTOM: RegexTree = RegexTree::Bottom;
const TOP: RegexTree = RegexTree::Top;

impl<'a> RegexTree<'a> {
    pub fn epsilon() -> &'a Self {
        &EPSILON
    }
    pub fn bottom() -> &'a Self {
        &BOTTOM
    }
    pub fn top() -> &'a Self {
        &TOP
